      ...
      {selected && (
        <div>
          ...
          <div className="mt-4 flex flex-wrap gap-1.5">
            {years.map((y) => (
              <button key={y.year} onClick={() => setSelectedYear(y.year)} className={`rounded-md px-2.5 py-1 font-mono text-xs ${y.year === selectedYear ? "bg-positive text-white" : "bg-neutral-100 text-neutral-600 dark:bg-neutral-800"}`}>{y.year}</button>
            ))}
          </div>
          <div className="mt-4 grid gap-6 md:grid-cols-2">
            <div>
              <h3 className="text-xs font-medium text-positive">Top inclusion</h3>
            ...
            <div>
              <h3 className="text-xs font-medium text-accent-text">Inclusion phrases</h3>
              ...
              {(data.phrases.lexicons?.civilizational?.length ?? 0) > 0 && (
                <>
                  <h3 className="mt-4 text-xs font-medium text-warning-text">
